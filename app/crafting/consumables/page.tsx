...
export default async function ConsumableCrafts() {
  const crafts = await getGameData('printing') as Craft[]
  const consumables = crafts.filter(craft => craft.type === 'consumable').sort(compareCraftRarity)

  return (
    <>
      <CraftingFilter />
      <ToggleListButton className={ styles.button } />
      <CraftingContainer crafts={ consumables } />
    </>
  )
}
